...
def main():
    ...
    print("")
    input("PRESS ENTER TO AGREE AND PROCEED")
    keytype = -1
    while keytype < 1 or keytype > 3:
        ...
        print("  2. Hex      (Only contains digits and characters A-F, may start with '0x')")
        print("  3. Wordlist (A series of English words)")
        keytype = int(input("> "))
    if keytype == 1:
        key = 0
        while key <= 0:
           inkey = input("Please enter the private key: ")
           inkey = inkey.strip()
           try:
        ...
        while phrase == []:
            ...
            lookup = {k: v for v, k in enumerate(words)}
            while word == None:
                word = input("Word %02d: (BLANK for END): " % (len(phrase) + 1))
                if word == "":
                    break
                else:
    ...
    print("")
    input("Please copy this information somewhere private. Press ENTER once done.")
    print("Complete!")
    return 0
